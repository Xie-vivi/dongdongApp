<template>
	<view class="comment-at-container">
		<!-- 顶部状态栏占位 -->
		...
		<view class="top-nav">
			<view class="nav-left" @click="goBack">
				<image src="/static/content/back.png" class="back-icon"></image>
			</view>
			<view class="nav-center">
				<text class="nav-title">评论@转发</text>
			</view>
		</view>
			...
			<view class="message-item" v-for="(message, index) in messageList" :key="index" @click="openMessage(message)">
				...
				<view class="message-content">
					<view class="message-header">
						...
						<text class="message-tag" v-if="message.tag">{{ message.tag }}</text>
					</view>
					<text class="message-action">{{ message.action }}</text>
					<text class="message-time">{{ message.time }}</text>
					
					<!-- 引用内容 -->
					<view class="quote-content" v-if="message.quote">
						<text class="quote-text">{{ message.quote }}</text>
					</view>
					
					<!-- 评论内容 -->
					<view class="comment-content" v-if="message.comment">
						<text class="comment-text">{{ message.comment }}</text>
					</view>
				</view>
				
				<!-- 相关图片 -->
				<view class="message-image" v-if="message.image">
					<image :src="message.image" class="post-img"></image>
				</view>
			</view>
		</view>
		...
		<view class="history-section">
			<text class="history-link">历史评论@转发</text>
		</view>
	</view>
</template>
...
.comment-content {
	background: rgba(138, 112, 201, 0.05);
	padding: 16rpx 20rpx;
	border-radius: 12rpx;
	border: 1rpx solid rgba(138, 112, 201, 0.2);
}
